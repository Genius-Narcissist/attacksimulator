...
const VALID_ROLES = ['admin', 'it_support', 'finance_manager', 'finance_analyst', 'hr_manager', 'general_employee']
...
function processEmployeeCSV(buffer) {
  ...
  const results = { valid: [], invalid: [], duplicateEmails: [] }
  ...
  for (let i = 0; i < parsed.data.length; i++) {
    ...
    if (seenEmails.has(clean.email)) {
      results.duplicateEmails.push({ row: rowNum, email: clean.email })
      continue
    }
